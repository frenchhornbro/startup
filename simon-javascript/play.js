...
setInterval(() => {
  let randPlayerNames = ['bob', 'steve', 'player', 'thing2', 'profjensen', 'IDK', 'Sally', 'Packmaster', 'Caleb', 'FocoMission'];

  const score = Math.floor(Math.random() * 3000);
  const newChatMsg = document.createElement('span');
  newChatMsg.className = 'player-event';
  newChatMsg.textContent = randPlayerNames[Math.floor(Math.random() * 9)];
  const newChatScore = document.createElement('span');
  newChatScore.textContent = ` scored ${score}`;
  const newChatContainer = document.createElement('div');
  newChatContainer.className = 'event';
  newChatContainer.appendChild(newChatMsg);
